<?php
if ( class_exists( 'GitlabPluginUpdater' ) ) {
	return;
}
class GitlabPluginUpdater {
    ...
    function __construct( $path, $repository, $token = "" ) {
    ...
    private function getRepositoryInfo() {
		...
		$url = "https://gitlab.com/api/v3/projects/{$this->repository}/repository/tags";
    ...
    public function setTransitent( $transient ) {
		...
		if ( ! empty( $this->data->name ) && ! empty( $transient->checked[$this->slug] ) ) {
			...
			$doUpdate = false;
			remove_filter( 'pre_set_site_transient_update_plugins', array( $this, "setTransitent" ) );
		}

		if ( $doUpdate ) {
			$package = "https://gitlab.com/api/v3/projects/{$this->repository}/repository/archive.zip?sha={$this->data->name}";
    ...
    public function setPluginInfo( $false, $action, $response ) {
		...
		$download = "https://gitlab.com/api/v3/projects/{$this->repository}/repository/archive.zip?sha={$this->data->name}";
